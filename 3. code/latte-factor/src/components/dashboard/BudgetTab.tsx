    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div>
        <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 4 }}>
          <span className="gradient-text">Ngân sách Tối ưu (0/1 Knapsack DP)</span>
        </h2>
        <p style={{ color: '#64748b', fontSize: 13 }}>
          Chọn danh mục chi tiêu để tối đa hóa độ thỏa mãn trong ngân sách {formatVND(budget)}/tháng
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16 }}>
        {[
          ...
          { label: 'Đã dùng', value: formatVND(totalWeight), color: '#f59e0b' },
          { label: 'Còn lại', value: formatVND(remaining), color: '#10b981' },
          { label: 'Độ thỏa mãn', value: `${totalValue} điểm`, color: '#8b5cf6' },
        ].map(item => (
      ...
      <div className="glass-card" style={{ padding: 20 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: '#94a3b8', marginBottom: 14 }}>
          ✅ Danh mục được chọn tối ưu ({selectedItems.length} danh mục)
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {selectedItems.map(item => (
              ...
              >
                ...
                <span style={{ flex: 1, fontSize: 14, fontWeight: 600, color: '#e2e8f0' }}>{item.name}</span>
